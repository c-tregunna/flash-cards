...
// General JS for site functionality
...
const card = document.querySelector('.card-outer')
...
function displayFact(facts) {
    ...
    // Clear any previous content and set up new question
    ...
    // Remove existing event listeners on the #reveal button
    const revealBtn = document.getElementById("reveal");
    revealBtn.removeEventListener("click", revealAnswer);
    revealBtn.addEventListener("click", revealAnswer);

    // Define the reveal answer function
...
subjects.addEventListener('click', (e) => {
    // Check if the clicked element is a subject with an id that matches a category
    const subjectId = e.target.id;
    if (categories[subjectId]) {
        // Hide welcome text and show flash card with a random question
        welcomeText.classList.add("hidden");
        flashCard.classList.remove("hidden");

        // Display a fact based on the selected category
        displayFact(categories[subjectId]);
    }
})
